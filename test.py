...
def get_board_meta(driver):
    tiles = driver.execute_script("""
        return Array.from(document.querySelectorAll('div.square'))
            .map(t => t.id).filter(id => /^\\d+_\\d+$/.test(id));
    """)
    rows = max(int(t.split('_')[0]) for t in tiles)
    cols = max(int(t.split('_')[1]) for t in tiles)

    mine_map = {(9, 9): 10, (16, 16): 40, (16, 30): 99}
    total_mines = mine_map.get((rows, cols), 40)

    center = f"{rows // 2}_{cols // 2}"
    print(f"  Board: {rows}x{cols} | Mines: {total_mines} | Opening tile: {center}")
    return total_mines, center


def make_left_click_script(tile_id):
    return (
        f"var e=document.getElementById('{tile_id}');if(!e)return;"
...
def run():
    ...
    total_mines, center_tile = get_board_meta(driver)
    left_click_start = make_left_click_script(center_tile)
